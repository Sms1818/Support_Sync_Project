...
interface SolutionResponse {
  ticket: any; // You might want to define a more specific type here
...
export default function TicketDetails({ ticket, projectKey }: TicketDetailsProps) {
  ...
  useEffect(() => {
    const fetchSolution = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`http://127.0.0.1:8000/tickets/solve/${ticket.issue_key}`, {
          method: "POST",
          headers: {
          ...
          <ReactMarkdown
            components={{
              ...
              code: ({ inline, className, children, ...props }: HTMLAttributes<HTMLElement> & { inline?: boolean }) => (
                <code
                  className={`${inline ? "bg-gray-200 rounded px-1" : "block bg-gray-200 p-2 rounded"} ${className}`}
                  {...props}
                >
                  {children}
                </code>
